// apps/web/src/lib/api.ts - ИСПРАВЛЕННАЯ ВЕРСИЯ
...
async function ensureCsrfToken(): Promise<string> {
    ...
    try {
        ...
        // ФИКС: правильный путь к токену
        const tokenFromBody = data?.data?.csrf;
        
        if (tokenFromBody && typeof tokenFromBody === 'string') {
            console.log('[CSRF] Using token from response body:', tokenFromBody.substring(0, 8) + '...');
...
// ... остальные функции (getJSON, deleteJSON) аналогично
...
// Добавьте также исправленные getJSON и deleteJSON

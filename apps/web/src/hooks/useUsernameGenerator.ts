...
type UsernameGeneratorInstance = {
	generate_username(): string
	generate_multiple?: (count: number) => unknown
	free?: () => void
}
...
let sharedGen: UsernameGeneratorInstance | null = null
let lastInitError: unknown = null

function isArrayLike(v: unknown): v is ArrayLike<unknown> {
	if (!v || typeof v !== 'object') return false
	if (Array.isArray(v)) return true
	return 'length' in v && typeof (v as { length?: unknown }).length === 'number'
}
...
async function importWasmGlue(): Promise<WasmModuleApi | null> {
	...
	wasmModOnce = (async () => {
		try {
			...
			// Динамический import по URL: TS это позволяет, но типы неизвестны — приводим вручную
			const modUnknown = (await import(
				/* webpackIgnore: true */ url
			)) as unknown
			const mod = modUnknown as WasmModuleApi
	...
	wasmReadyOnce = (async () => {
		...
		try {
			...
			if (typeof mod.UsernameGenerator === 'function') {
				...
				sharedGen = {
					generate_username: () => mod.generate_username?.() ?? 'User',
					generate_multiple: (n: number) =>
						Array.from(
							{ length: Math.min(Math.max(n | 0, 0), 50) },
							() => mod.generate_username?.() ?? 'User',
						),
				}
			} else {
...
class FallbackGenerator implements UsernameGeneratorInstance {
	...
	generate_username(): string {
		const syllables = 2 + this.rng.range(2) // 2..3 слога
		let out = ''
		...
		for (let i = 0; i < syllables; i++) {
			...
			if (i === 0) {
				const firstChunk = onset ? onset : nuc
				out += firstChunk.charAt(0).toUpperCase() + firstChunk.slice(1)
				out += (onset ? nuc : '') + coda
			} else {
				out += onset + nuc + coda
			}
...
function getGeneratorSync(): UsernameGeneratorInstance {
	return sharedGen ?? new FallbackGenerator()
}

export function useUsernameGenerator() {
	const [ready, setReady] = useState(false)
	const localGenRef = useRef<UsernameGeneratorInstance | null>(null)
	...
	const generateMultiple = useCallback((count: number): string[] => {
		try {
			...
			const r = g.generate_multiple ? g.generate_multiple(count) : undefined

			if (Array.isArray(r)) {
				return r.map(String)
			}

			if (isArrayLike(r) && typeof r !== 'string') {
				return Array.from(r, v => String(v))
			}
			...
			return (localGenRef.current.generate_multiple?.(count) as string[]) ?? []
		}
	}, [])

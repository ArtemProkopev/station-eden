// apps/web/src/components/Game/GameSession.tsx
'use client'
...
type CardType = 'profession' | 'health' | 'trait' | 'secret' | 'role' | 'resource' | 'gender' | 'age' | 'body'
...
export default function GameSession({ gameId, userId, username }: GameSessionProps) {
  const [gameState, setGameState] = useState<any>(null)
  const [myCards, setMyCards] = useState<Record<string, CardDetails>>({})
  const [selectedVote, setSelectedVote] = useState<string>('')
  const [phaseTimeLeft, setPhaseTimeLeft] = useState<number>(0)
  const [chatMessages, setChatMessages] = useState<Array<{player: string, text: string, isSystem?: boolean}>>([])
  ...
  const [revealedPlayer, setRevealedPlayer] = useState<{name: string, cards: any} | null>(null)
  ...
  const handleWebSocketMessage = useCallback((data: any) => {
    ...
    switch (data.type) {
      ...
      case 'CRISIS_SOLVED':
        addToChat('Система', `Кризис "${data.crisis}" решен игроком ${data.playerName}!`, true)
        break
      ...
      case 'PLAYER_EJECTED':
        addToChat('Система', `Игрок ${data.playerName} выбыл с ${data.votes} голосами!`, true)
        break
      ...
      case 'CARD_REVEALED':
        addToChat('Система', `${data.playerName} раскрыл(а) карту: ${getCardDisplayName(data.cardType, data.cardId)}`, true)
        break
        
      case 'PLAYER_VOTED':
        addToChat('Система', `${data.voterName} проголосовал(а) против ${data.targetName}`, true)
        break
        
      case 'VOTE_REQUESTED':
        addToChat('Система', `${data.playerName} запросил(а) голосование (${data.voteCount}/${data.requiredCount})`, true)
        break
      ...
      case 'PLAYER_INFECTED':
        addToChat('Система', `Игрок ${data.playerName} заражен игроком ${data.infectedByName}!`, true)
        break
      ...
      case 'GAME_FINISHED':
        ...
        setGameState((prev: any) => prev ? { ...prev, phase: 'game_over' } : null)
        break
  ...
  const addToChat = (player: string, text: string, isSystem: boolean = false) => {
  ...
  const getCardDisplayName = (cardType: string, cardId: string): string => {
  ...
  const getServerCardType = (clientType: CardType): string => {
    switch (clientType) {
      ...
      case 'role': return 'hiddenRole'
      case 'resource': return 'resource'
      case 'gender': return 'gender'
      case 'age': return 'age'
      case 'body': return 'bodyType'
  ...
  const getPhaseName = (phase: GamePhase): string => {
    switch (phase) {
      case 'introduction': return 'Введение'
      ...
      case 'crisis': return 'Кризис'
      case 'intermission': return 'Между раундами'
  ...
  const getPhaseDescription = (phase: GamePhase): string => {
    switch (phase) {
      case 'introduction': return 'Просмотр заставки и ознакомление с сюжетом'
      case 'preparation': return 'Изучите свои карты и подготовьтесь к обсуждению'
      case 'discussion': return 'Обсуждайте с другими игроками, кто подозрителен'
      case 'voting': return 'Голосуйте за исключение игрока'
      case 'reveal': return 'Раскрытие карт выбывшего игрока'
      case 'crisis': return 'Решение кризиса на станции'
      case 'intermission': return 'Подготовка к следующему раунду'
      case 'game_over': return 'Игра завершена'
      default: return ''
    }
  }
          ...
          {Object.entries(myCards).map(([type, card]) => (
            <div key={type} className={styles.cardItem}>
              ...
              <button 
                ...
                disabled={gameState?.phase !== 'discussion' || !gameState?.players?.find((p: any) => p.id === userId)?.isAlive}
              >
                Раскрыть карту
              </button>
            </div>
    ...
    <div className={styles.modalOverlay}>
      <div className={styles.modalContent}>
        ...
        {revealedPlayer?.cards && (
          <div className={styles.cardsGrid}>
            {Object.entries(revealedPlayer.cards).map(([type, card]: [string, any]) => (
              card && (
                <div key={type} className={styles.cardItem}>
                  <h3>{getCardTypeName(type as CardType)}</h3>
                  <h4>{card.name}</h4>
      ...
      <div className={styles.modalContent}>
        <div className={styles.crisisAlert}>
          ...
          <div className={styles.crisisInfo}>
            <p><strong>Тип:</strong> {activeCrisis?.type === 'technological' ? 'Технологический' : 
                                     activeCrisis?.type === 'biological' ? 'Биологический' : 
                                     'Внешняя угроза'}</p>
            <p><strong>Штраф:</strong> {activeCrisis?.penalty}</p>
            <p><strong>Время на решение:</strong> {formatTime(phaseTimeLeft)}</p>
            {activeCrisis?.priorityProfessions && activeCrisis.priorityProfessions.length > 0 && (
              <p><strong>Подходящие профессии:</strong> {activeCrisis.priorityProfessions.join(', ')}</p>
            )}
          </div>
          ...
          <div className={styles.crisisActions}>
            ...
            <p className={styles.crisisHint}>
              {activeCrisis?.type === 'technological' ? 'Инженеры и техники наиболее эффективны' :
               activeCrisis?.type === 'biological' ? 'Медики и биологи могут помочь' :
      ...
      <div className={styles.modalContent}>
        <div className={styles.resultsContent}>
          ...
          {gameResults?.reason === 'hidden_role_win' && (
            <p className={styles.resultReason}>Скрытая роль выполнила свою миссию!</p>
          )}
            ...
            {gameResults?.scores
              ...
              .map((player: any) => (
                <div key={player.id} className={styles.winnerItem}>
                  ...
                  <span className={styles.winnerScore}>{player.score} очков</span>
                </div>
              ))}
          </div>
          ...
          <div className={styles.phaseActions}>
            <div className={styles.discussionActions}>
              ...
              <button 
                ...
                disabled={!currentPlayer?.isAlive || gameState.voteRequests?.includes(userId)}
              >
                Начать голосование ({gameState.voteTriggerCount || 0}/{requiredVotes})
              </button>
              ...
              {currentPlayer?.hiddenRole === 'role_saboteur' && !currentPlayer?.hasUsedAbility && (
                <button 
                  className={styles.abilityButton}
              ...
              {currentPlayer?.gender === 'gender_nonbinary' && !currentPlayer?.hasUsedAbility && (
                <button 
                  className={styles.abilityButton}
              ...
              {currentPlayer?.hiddenRole === 'role_xenophag' && !currentPlayer?.hasUsedAbility && (
                <button 
                  className={styles.abilityButton}
                  onClick={() => {
                    const targetId = alivePlayers.find((p: any) => p.id !== userId)?.id
            ...
            <div className={styles.voteInfo}>
              <p>Голосование начнется, когда {requiredVotes} игроков нажмет кнопку</p>
            </div>
          </div>
        )
          ...
          <div className={styles.phaseActions}>
                ...
                .map((player: any) => (
                  ...
                  >
                    <div className={styles.votePlayerInfo}>
                      ...
                      {player.isInfected && <span className={styles.infectedBadge}>🦠</span>}
                      {player.isSuspicious && <span className={styles.suspiciousBadge}>👁️</span>}
                    </div>
            ...
            {currentPlayer?.vote && (
              <p className={styles.voteConfirmed}>
                Вы проголосовали против {gameState.players?.find((p: any) => p.id === currentPlayer.vote)?.name}
              </p>
            )}
          </div>
        )
        ...
        return (
          <div className={styles.phaseActions}>
            <button 
              ...
              onClick={() => setActiveCrisis(gameState.currentCrisis || activeCrisis || {})}
            >
              Просмотреть кризис
            </button>
        ...
        <div className={styles.playersList}>
          ...
          {gameState.players?.map((player: any) => (
            <div key={player.id} className={styles.waitingPlayer}>
              ...
              {player.id === gameState.creatorId && <span className={styles.creatorBadge}>👑</span>}
            </div>
          ))}
        </div>
      ...
      <header className={styles.header}>
        <div className={styles.gameTitle}>
          ...
          <div className={styles.gameSubtitle}>
            <span className={styles.round}>Раунд {gameState.round || 1}/{gameState.maxRounds || 10}</span>
            <span className={styles.phase}>Фаза: {getPhaseName(gameState.phase)}</span>
          </div>
        </div>
        ...
        <div className={styles.gameStats}>
          <div className={styles.statItem}>
            ...
            <span className={styles.statValue}>⏱️ {formatTime(phaseTimeLeft)}</span>
          </div>
          <div className={styles.statItem}>
            <span className={styles.statLabel}>Капсула:</span>
            <span className={styles.statValue}>
              🚀 {gameState.occupiedSlots || 0}/{gameState.capsuleSlots || Math.floor(gameState.players?.length / 2)} мест
            </span>
          </div>
          <div className={styles.statItem}>
          ...
          {currentPlayer && (
            <div className={styles.statItem}>
              ...
              <span className={styles.statValue}>🏆 {currentPlayer.score || 0}</span>
            </div>
          )}
        </div>
                ...
                <div className={styles.playerHeader}>
                  ...
                  <div className={styles.playerInfo}>
                    ...
                    <div className={styles.playerStatus}>
                      ...
                      {player.isInfected && <span className={styles.infected}>🦠 Заражен</span>}
                      {player.isSuspicious && <span className={styles.suspicious}>👁️ Подозрителен</span>}
                    </div>
                  ...
                  <div className={styles.playerBadges}>
                    {player.isCaptain && <span className={styles.captainBadge}>👑</span>}
                    {player.isSeniorOfficer && <span className={styles.officerBadge}>⭐</span>}
                    {player.revealedCards > 0 && (
                      <span className={styles.revealedBadge}>📄{player.revealedCards}</span>
                    )}
                  </div>
                </div>
                
                {gameState.phase === 'voting' && player.isAlive && player.id !== userId && (
            ...
            <div className={styles.phaseTimer}>
              <div className={styles.timerBar}>
                <div 
                  ...
                  style={{ width: `${(phaseTimeLeft / (gameState.phaseDuration || 180)) * 100}%` }}
                ></div>
              </div>
              <span className={styles.timerText}>
                {formatTime(phaseTimeLeft)} / {formatTime(gameState.phaseDuration || 180)}
              </span>
            </div>
          </div>
        ...
        <div className={styles.playerStatusInfo}>
          {currentPlayer && (
            <>
              ...
              {currentPlayer.isInfected && <span className={styles.infectionWarning}>🦠 Заражен!</span>}
              {currentPlayer.isSuspicious && <span className={styles.suspiciousWarning}>👁️ Под подозрением</span>}
              <span> | Карт раскрыто: {currentPlayer.revealedCards || 0}</span>
              <span> | Голосов для голосования: {gameState.voteTriggerCount || 0}/{requiredVotes}</span>
            </>
          )}
        </div>
        
        <div className={styles.gameRules}>
          <button className={styles.rulesButton} onClick={() => window.open('/rules', '_blank')}>
            📖 Правила
          </button>
          <button className={styles.settingsButton} onClick={() => alert('Настройки игры')}>
            ⚙️ Настройки
          </button>

...
export class VoiceService {
	...
	constructor(private readonly cfg: ConfigService) {
		...
		this.logger.log(
			`[LiveKit config] url=${this.url} key=${this.apiKey ? 'SET' : 'EMPTY'} secret=${this.apiSecret ? 'SET' : 'EMPTY'}`
		)
	}
	...
	async createToken(lobbyId: string, user: VoiceUser | undefined) {
		...
		if (!this.apiKey || !this.apiSecret || !this.url) {
			this.logger.error(
				`LiveKit env missing: url=${this.url} apiKey=${this.apiKey ? 'SET' : 'EMPTY'} apiSecret=${this.apiSecret ? 'SET' : 'EMPTY'}`
			)
